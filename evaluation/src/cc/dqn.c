...
#ifdef DEBUG
#define DBG(...) fprintf(stderr, "[dqn] " __VA_ARGS__)
#else
#define DBG(...) ((void)0)
#endif
...
/* Map discrete action index to log2 cwnd scale factor (mirrors dqn_trainer.py) */
static float discrete_to_scale(int action_idx, float cwnd_mss) {
    float c = cwnd_mss < 1.0f ? 1.0f : cwnd_mss;
    switch (action_idx) {
        case 0: return  1.0f;                                        /* cwnd * 2    */
        case 1: { float t = c - 1.0f < 1.0f ? 1.0f : c - 1.0f;
                  float v = log2f(t / c);
                  return v < -1.0f ? -1.0f : (v > 1.0f ? 1.0f : v); } /* cwnd - MSS */
        case 2: return  0.0f;                                        /* no change   */
        case 3: { float t = c + 1.0f;
                  float v = log2f(t / c);
                  return v < -1.0f ? -1.0f : (v > 1.0f ? 1.0f : v); } /* cwnd + MSS */
        case 4: return -1.0f;                                        /* cwnd / 2    */
        case 5: { float v = log2f(1.0f / c);
                  return v < -1.0f ? -1.0f : (v > 1.0f ? 1.0f : v); } /* cwnd = MSS */
        default: return 0.0f;
    }
}

static int read_info(const char *model_dir, int *k_out, int *num_actions_out) {
    char path[MAX_PATH];
    snprintf(path, sizeof(path), "%s/dqn.info.json", model_dir);

    FILE *f = fopen(path, "r");
    if (!f) {
        fprintf(stderr, "[dqn] cannot open %s\n", path);
    ...
    char buf[256];
    int k = -1, num_actions = NUM_ACTIONS;
    while (fgets(buf, sizeof(buf), f)) {
        char *p;
        if ((p = strstr(buf, "\"k\"")) && (p = strchr(p, ':')))
            k = atoi(p + 1);
        if ((p = strstr(buf, "\"num_actions\"")) && (p = strchr(p, ':')))
            num_actions = atoi(p + 1);
    }
    fclose(f);
    *k_out = k;
    *num_actions_out = num_actions;
    DBG("loaded k=%d num_actions=%d from %s\n", k, num_actions, path);
    return 0;
}

static void dqn_run_inference(dqn_ctx_t *ctx) {
    ...
    if (!state || !entries) {
        fprintf(stderr, "[dqn] OOM in inference\n");
    ...
    if (status) {
        fprintf(stderr, "[dqn] ORT error: %s\n", g_ort->GetErrorMessage(status));
    ...
    float *q_data = NULL;
    status = g_ort->GetTensorMutableData(output, (void **)&q_data);
    if (status) {
        fprintf(stderr, "[dqn] ORT error: %s\n", g_ort->GetErrorMessage(status));
        g_ort->ReleaseStatus(status);
        goto cleanup;
    ...
    /* argmax */
    int best = 0;
    for (int i = 1; i < ctx->num_actions; i++)
        if (q_data[i] > q_data[best]) best = i;

    float scale = discrete_to_scale(best, cwnd_mss);
    float new_cwnd = ctx->current_cwnd * powf(2.0f, scale);
    new_cwnd = fmaxf((float)MSS, fminf(1000.0f * (float)MSS, new_cwnd));

    DBG("best_action=%d scale=%.4f old_cwnd=%.0f new_cwnd=%.0f\n",
        best, scale, ctx->current_cwnd, new_cwnd);
...
static void dqn_on_ack(void *raw_ctx, tcp_info_t *info) {
    dqn_ctx_t *ctx = (dqn_ctx_t *)raw_ctx;
    ...
    dqn_run_inference(ctx);
    ctx->inference_busy = 0;
}

static void dqn_on_timeout(void *raw_ctx, tcp_info_t *info) {
    dqn_on_ack(raw_ctx, info);
}

static float dqn_get_cwnd(void *raw_ctx) {
    return ((dqn_ctx_t *)raw_ctx)->current_cwnd;
}

static void dqn_destroy(void *raw_ctx) {
    dqn_ctx_t *ctx = (dqn_ctx_t *)raw_ctx;
...
static cc_ops_t dqn_ops = {
    .on_ack     = dqn_on_ack,
    .on_timeout = dqn_on_timeout,
    .get_cwnd   = dqn_get_cwnd,
    .destroy    = dqn_destroy,
};

cc_ops_t *dqn_create(void **ctx_out, const char *model_dir) {
    g_ort = OrtGetApiBase()->GetApi(ORT_API_VERSION);
    if (!g_ort) {
        fprintf(stderr, "[dqn] failed to get ONNX Runtime API\n");
        return NULL;
    }

    int k = -1, num_actions = NUM_ACTIONS;
    if (read_info(model_dir, &k, &num_actions) < 0 || k <= 0) {
        fprintf(stderr, "[dqn] invalid k=%d from %s\n", k, model_dir);
        return NULL;
    }

    dqn_ctx_t *ctx    = calloc(1, sizeof(dqn_ctx_t));
    ctx->k            = k;
    ctx->num_actions  = num_actions;
    ctx->current_cwnd = (float)(INIT_CWND_SEGS * MSS);
    ctx->queue        = queue_create(k);

    OrtStatus *s;
    s = g_ort->CreateEnv(ORT_LOGGING_LEVEL_WARNING, "dqn_eval", &ctx->env);
    if (s) g_ort->ReleaseStatus(s);
    s = g_ort->CreateSessionOptions(&ctx->opts);
    if (s) g_ort->ReleaseStatus(s);

    char onnx_path[MAX_PATH];
    snprintf(onnx_path, sizeof(onnx_path), "%s/dqn.onnx", model_dir);
    DBG("loading model from %s\n", onnx_path);

    OrtStatus *status = g_ort->CreateSession(ctx->env, onnx_path,
                                             ctx->opts, &ctx->session);
    if (status) {
        fprintf(stderr, "[dqn] CreateSession failed: %s\n",
                g_ort->GetErrorMessage(status));
        g_ort->ReleaseStatus(status);
        queue_destroy(ctx->queue);
    ...
    DBG("model loaded. k=%d num_actions=%d state_dim=%d\n",
        k, num_actions, k * 3 + 2);
